...
class PyTux:
    ...
    def rollout(self, track, controller, planner=None, max_frames=1000, verbose=False, data_callback=None, road_classifier=None, gray_scale_road_classifier=None):
        """
        ...
        for t in range(max_frames):
            ...
            conerning_object_in_frame = None
            """
            if road_classifier:
                image = np.array(self.k.render_data[0].image)
                pred = road_classifier(TF.to_tensor(image)[None])
                _, predicted = torch.max(pred, dim=1)
                concerning_object_in_frame = predicted.item()
                print(predicted.item())
                current_vel = np.linalg.norm(kart.velocity)
                action = controller(aim_point_image, current_vel, concerning_object_in_frame=concerning_object_in_frame)
            ...
            if gray_scale_road_classifier:
                image = np.array(Image.fromarray(self.k.render_data[0].image).convert("L"))
                pred = gray_scale_road_classifier(TF.to_tensor(image)[None])
                _, predicted = torch.max(pred, dim=1)
                concerning_object_in_frame = predicted.item()
                print(predicted.item())
                current_vel = np.linalg.norm(kart.velocity)
            ...
            if verbose:
                ...
                if planner:
                    ...
                    #ax.add_artist(plt.Circle(WH2*(2+aim_point_image), 2, ec='y', fill=False, lw=1.5))
                plt.pause(1e-3)
